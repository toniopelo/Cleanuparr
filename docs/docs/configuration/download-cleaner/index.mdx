...
Optional tag/label source filter for unlinked downloads. When configured, torrents must have at least one listed tag/label in addition to matching an unlinked category. For Transmission, labels are matched while the category remains the path-derived category.
...
Optional tag/label source filter for unlinked downloads. When configured, torrents must have every listed tag/label in addition to matching an unlinked category.
